 ...
 * @author jongmin
 */
public class ReadMailHandler extends HttpServlet implements java.io.Serializable{
    ...
    private void download(HttpServletRequest request, HttpServletResponse response) { //throws IOException {
        ...
        try {
            ...
            // AvoidFileStream 수정(Avoid instantiating Fileinputstream)
            try (InputStream fis = Files.newInputStream(Paths.get(fileName))) {
                fis.read(b);
            } // ; 오타 추가되어 있던거 수정

            sos = response.getOutputStream(); //다운로드
            sos.write(b);
            sos.flush();
            sos.close();
