 ...
 * @author jongmin
 */
public class MessageParser { // 코드의 복잡성이 2 (최고는 11) 경고 문구
    ...
    public boolean parse(boolean parseBody) {
        ...
        try {
            ...
            return status; //finaly 블록에서 리턴하는 것을 자제하라는 경고 문구
        }
    }
    ...
    private void getPart(Part p) throws Exception { // if, while, for, case label 갯수가 13+ 이라 복잡성이 높다.
                                                      // 코드의 복잡성이 11 경고 문구
        ...
        if (disp != null && (disp.equalsIgnoreCase(Part.ATTACHMENT)
            ...
            if (fileName != null) {
                ...
                // AvoidFileStream(FileOutputStream)
                OutputStream fos = Files.newOutputStream(Paths.get(tempUserDir + File.separator + filename));
                dh.writeTo(fos);
                fos.flush();
                fos.close();
            }
