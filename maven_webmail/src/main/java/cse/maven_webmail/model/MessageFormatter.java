...
public class MessageFormatter {
    ...
    public String getMessageTable(Message[] messages) {
        StringBuilder buffer = new StringBuilder(500); //InsufficientString
...
//        return "MessageFormatter 테이블 결과";
    ...
    public String getMessage(Message message) {
        StringBuilder buffer = new StringBuilder(300); //InsufficientStringBufferDeclaration

        // MessageParser parser = new MessageParser(message, userid);
        MessageParser parser = new MessageParser(message, userid, request);
        parser.parse(true);

        buffer.append("보낸 사람: " + parser.getFromAddress() + " <br>"); // 문자열을 반복하는 행동으로 인한 경고문구
